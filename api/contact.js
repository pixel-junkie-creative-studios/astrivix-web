...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // Directly configured Gmail App Password: zmrm yuff wzwx bpkp
    const gmailPass = (context.env?.GMAIL_APP_PASSWORD || 'zmrmyuffwzwxbpkp').replace(/\s+/g, '');
    const gmailUser = context.env?.GMAIL_USER || 'business@astrivix.in';
    ...
    // Mail A: Admin Inquiry Notification to business@astrivix.in
    ...
    // Mail B: Client Automatic Receipt to client's email
      ...
      html: `
        <div style="font-family: system-ui, -apple-system, sans-serif; max-width: 600px; margin: 0 auto; background: #050508; color: #ffffff; padding: 32px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.1);">
          ...
          <p style="color: rgba(255,255,255,0.8); font-size: 14px; line-height: 1.6; margin-bottom: 20px;">
            Thank you for reaching out to <strong>Astrivix Corp</strong>. We have received your project inquiry and registered your brief under reference <strong style="color: #38bdf8; font-family: monospace;">#${inquiryRef}</strong>.
          </p>

          <div style="background: #0e0e16; padding: 20px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 24px;">
            <div style="font-family: monospace; font-size: 11px; color: #34d399; text-transform: uppercase; margin-bottom: 12px;">✓ INQUIRY SUMMARY CONFIRMED</div>
          ...
          <div style="background: rgba(56,189,248,0.08); border: 1px solid rgba(56,189,248,0.2); padding: 16px; border-radius: 12px; margin-bottom: 24px; text-align: center;">
            <p style="margin: 0; font-size: 13px; font-family: monospace; color: #38bdf8; font-weight: 600;">GUARANTEE: PERSONALIZED RESPONSE WITHIN 2 HOURS</p>
            <p style="margin: 6px 0 0 0; font-size: 12px; color: rgba(255,255,255,0.6); font-family: sans-serif;">Our team is reviewing your requirements and will reach out with a detailed roadmap.</p>
          </div>
    ...
    return new Response(JSON.stringify({ error: err.message || 'Mail dispatch error' }), {
      status: 500,
      headers: corsHeaders
    });
  }
